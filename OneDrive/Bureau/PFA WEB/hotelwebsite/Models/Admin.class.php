<?php
    ...
    class Admin extends Dbc
    {
        ...
        public function update_room($roomName, $roomPrice, $roomId)
        {
            $sql = "UPDATE rooms SET room_name = ?, room_price = ? WHERE room_id = ?";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute([$roomName, $roomPrice, $roomId]);
        }
        
        public function update_pension($pensionname, $pensionprice, $pensionID)
        {
            $sql = "UPDATE pension SET pension_name = ?, pension_price = ? WHERE pension_id = ?";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute([$pensionname, $pensionprice, $pensionID]);
        }

        public function add_room($roomname, $roomprice)
        {
            $sql = "INSERT INTO rooms (room_name, room_price) VALUES (?, ?)";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute([$roomname, $roomprice]);
        }
